...
| `sort`     | N/A     | If specified, returns findings sorted by one of the chosen parameters: `severity`, `impact` or `state`. When sort parameter is set, the field will be returned in ascending order, to return as descending order, use `-` before the chosen field. Example: `https://api.cobalt.io/findings?sort=-severity`. |
