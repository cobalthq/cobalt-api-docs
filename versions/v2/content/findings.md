...
| `state`    | N/A     | If specified, returns findings that match `state`. See Response Fields below for example `state` values. Example: `https://api.cobalt.io/findings?state=check_fix`. Returns an empty list if `state` value isn't one of the filter values listed below in Response Fields.          |
| `severity` | N/A     | If specified, returns findings that match `severity`. See Response Fields below for example `severity` values. Example: `https://api.cobalt.io/findings?severity=medium`. Returns an empty list if `severity` value isn't one of the filter values listed below in Response Fields. |
