...
| `sort`     | N/A     | If specified, returns findings sorted by one of the chosen parameters: `severity`. `impact` or `state`. By default, it returns in ascending order, to return as descending order, use `-` before the chosen field, for example `https://api.cobalt.io/findings?sort=-severity`. |
