...
| `state`    | N/A     | If specified, returns findings that match `state`. See Response Fields below for example `state` values. Example: `https://api.cobalt.io/findings?state=check_fix`. Using any filter value other than what is listed in Response Fields returns an empty list.       |
| `severity` | N/A     | If specified, returns findings that match `severity`. See Response Fields below for example `severity` values. Example: `https://api.cobalt.io/findings?severity=medium`. Using any filter value other than what is listed in Response Fields returns an empty list. |
